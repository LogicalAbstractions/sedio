    ...
    public class PolicyHttpMessageHandler : DelegatingHandler
    {
        ...
        protected override async Task<HttpResponseMessage> SendAsync(HttpRequestMessage request, CancellationToken cancellationToken)
        {
            ...
            finally
            {
                if (cleanUpContext)
                {
                    request.CleanupPolicyExecutionContext();
                }
            }
        ...
        {
            if (_policySelector != null)
            {
                var policy = _policySelector(request);
                if (policy == null)
                {
                    throw new InvalidOperationException("PolicySelector returned null");
                }

                return policy;
            }

            throw new InvalidOperationException("No PolicySelector specified");
        }
    }
}
